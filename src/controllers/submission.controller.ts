...
import { runQueue, submitQueue, runQueueEvents, submitQueueEvents } from '../queues/queues';
  ...
  try {
    ...
    if ((passedTests === question.testCases.length) && match) {
      const acceptedSubmissions = await prisma.submission.findMany({
        where: { userId, matchId, status: 'ACCEPTED' },
