...
import { runQueue, submitQueue, submitQueueEvents, runQueueEvents } from '../../queues/queues';
  ...
  try {
    ...
    const contest = await prisma.contest.findFirst({
      ...
      include: {
        questions: {
          where: { id: questionId },
          select: { score: true, testCases: true, timeLimit: true }
        }
      }
    });
    ...
    const userPreviousSubmission = await prisma.submission.findFirst({
      where: { contestId, userId, questionId, status: "ACCEPTED" }
    });
    ...
    if (isAccepted && userPreviousSubmission === null) {
      await Promise.all([
        prisma.contestParticipation.update({
          where: { userId_contestId: { userId, contestId } },
          data: { score: { increment: question.score } }
        }),
        prisma.contestLeaderboard.upsert({
          where: { contestId_userId: { contestId, userId } },
          create: { userId, contestId, score: question.score, problemsSolved: 1, lastSubmissionTime: new Date() },
          update: { score: { increment: question.score }, problemsSolved: { increment: 1 }, lastSubmissionTime: new Date() }
        })
      ]);
    }
